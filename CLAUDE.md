...
20. PM2 restart kuralı: ecosystem.config.cjs veya .env değişikliğinde sadece pm2 restart YETERSİZ (PM2 ENV'yi cache'ler). Çözüm: ./scripts/pm2-reload-env.sh kullan veya pm2 delete + pm2 start yap. ENV değişikliği sonrası API çalışmıyorsa bu kurala bak. DEĞİŞMEZ.
34. Git Post-Merge Hook: Sunucuda .git/hooks/post-merge otomatik ENV reload yapıyor. git pull sonrası .env veya ecosystem.config.cjs değiştiyse pm2-reload-env.sh otomatik çalışır. Manuel müdahale gerekmez. Hook konumu: /var/www/hylilabs/.git/hooks/post-merge. DEĞİŞMEZ.
