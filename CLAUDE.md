...
38. Sidebar Toggle (14.03.2026): Sidebar tüm sayfalarda açılır/kapanır. Global Header authenticated-layout.tsx'te tanımlı (SidebarTrigger + ThemeSwitch + ProfileDropdown). Sayfa bazlı Header kullanımı kaldırıldı (dashboard, settings, tasks, users, apps, chats). Sidebar durumu cookie ile persist ediliyor (sidebar_state). DEĞİŞMEZ.
