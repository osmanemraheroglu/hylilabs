...
def create_pool(body: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # === FAZ 10.2: Pozisyon oluşturulunca otomatik V3 eşleştirme ===
        transferred = 0
        if pool_type == "position" and new_id:
            try:
                from database import pull_matching_candidates_to_position
                result = pull_matching_candidates_to_position(new_id, company_id)
                transferred = result.get("transferred", 0)
                if transferred > 0:
                    print(f"[create_pool] Pozisyon {new_id} oluşturuldu, {transferred} aday otomatik eşleştirildi")
            except Exception as pull_err:
                print(f"[create_pool] pull_matching hatası (devam ediliyor): {pull_err}")
        # === FAZ 10.2 SONU ===

        return {"success": True, "id": new_id, "transferred": transferred, "message": f"Pozisyon oluşturuldu, {transferred} aday eşleşti"}
    ...
    try:
        ...
        if is_system:
            ...
            if candidates:
                ...
                # FAZ 14: Pozisyon lokasyonunu al
                pos_location = ""
                with get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("SELECT lokasyon FROM department_pools WHERE id = ?", (pool_id,))
                    pos_row = cursor.fetchone()
                    if pos_row:
                        pos_location = pos_row["lokasyon"] or ""
                
                # FAZ 14: location_status fallback fonksiyonu
                def get_location_status_fallback(cand_loc: str, pos_loc: str) -> dict:
                    ...
                    cand_lower = cand_loc.lower().strip()
                    pos_lower = pos_loc.lower().strip()
                    
                    # Tam eşleşme
                    if cand_lower == pos_lower:
                        return {
                            'status': 'green',
                            'candidate_location': cand_loc,
                            'position_location': pos_loc,
                            'match_type': 'Aynı şehir'
                        }
                    
                    # Kısmi eşleşme (biri diğerini içeriyor)
                    if cand_lower in pos_lower or pos_lower in cand_lower:
                        return {
                            'status': 'yellow',
                            'candidate_location': cand_loc,
                            'position_location': pos_loc,
                            'match_type': 'Yakın lokasyon'
                        }
                    
                    # İstanbul özel durumu
                    istanbul_ilceler = ['kadıköy', 'beşiktaş', 'şişli', 'üsküdar', 'ataşehir', 'maltepe', 'kartal', 'pendik', 'bakırköy', 'bahçelievler', 'bağcılar', 'esenyurt', 'beylikdüzü', 'avcılar', 'küçükçekmece', 'büyükçekmece', 'başakşehir', 'sarıyer', 'beykoz', 'fatih', 'beyoğlu', 'zeytinburnu', 'güngören', 'esenler', 'gaziosmanpaşa', 'sultangazi', 'eyüp', 'kağıthane', 'tuzla', 'sultanbeyli', 'sancaktepe', 'çekmeköy', 'arnavutköy', 'silivri', 'çatalca', 'şile', 'adalar']
                    cand_istanbul = 'istanbul' in cand_lower or any(ilce in cand_lower for ilce in istanbul_ilceler)
                    pos_istanbul = 'istanbul' in pos_lower or any(ilce in pos_lower for ilce in istanbul_ilceler)
                    
                    if cand_istanbul and pos_istanbul:
                        return {
                            'status': 'yellow',
                            'candidate_location': cand_loc,
                            'position_location': pos_loc,
                            'match_type': 'Aynı şehir (İstanbul)'
                        }
                ...
                with get_connection() as conn:
                    ...
                    for c in candidates:
                        try:
                            ...
                            # FAZ 14: Fallback - matches'ta yoksa dinamik hesapla
                            if not location_status or not location_status.get("status"):
                                cand_loc = c.get("lokasyon", "") or ""
                                location_status = get_location_status_fallback(cand_loc, pos_location)
                            
                            c["location_status"] = location_status
                        except Exception as e:
        ...
        with get_connection() as conn:
            ...
            if ai_row:
                try:
                    ...
                    # FAZ 13.4: scores ve layer_scores (alias) dondur
                    ...
                    ai_eval = {
                        ...
                        "scores": scores_data,
                        "layer_scores": scores_data
                    }
                except json.JSONDecodeError:
                    ai_eval = {"text": ai_row[0], "v2_score": ai_row[1], "date": ai_row[2]}

        # FAZ 13.5: V2/V3 ayri gosterim icin agirlik bilgisi
        scoring_info = {
            "v2_score": candidate.get("v2_score", 0),
