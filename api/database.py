...
def match_single_candidate_to_positions(candidate_id: int, company_id: int) -> dict:
    ...
    for position in positions:
        ...
        try:
            v2_result = calculate_match_score_v2(candidate_dict, position_dict)
            match_score = v2_result.get('total', title_score) if v2_result else title_score
        except Exception as e:
        ...
        # Pozisyona ekle
        result = add_candidate_to_position(candidate_id, position_id, match_score, company_id)
        ...
        if result.get('success'):
            ...
            logger.info(f"match_single_candidate: Aday {candidate_id} -> Pozisyon {position_id} ({position_name}), skor={match_score}")
            break
