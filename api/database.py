...
@contextmanager
def get_connection():
    """Veritabani baglantisi context manager (READ için optimize)

    Performans iyileştirmeleri:
    - WAL mode: Concurrent read/write performansını artırır
    - Timeout: Database locked hatalarını önler

    NOT: WRITE işlemleri için get_write_connection() kullanın.
    """
    ensure_data_dir()
...
def get_write_connection():
    """WRITE işlemleri için thread-safe connection (WRITE_LOCK korumalı)

    READ/WRITE Separation (18.03.2026):
    - WRITE_LOCK ile aynı anda tek writer
    - Exception durumunda rollback
    - Başarılı durumda commit
    - row_factory ve TURKISH_LOWER dahil
...
def init_database():
    ...
    with get_write_connection() as conn:
        ...
        # Candidate Intelligence tablosu (CV Profil Analizi)
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS candidate_intelligence (
                ...
                relocation_willing INTEGER DEFAULT 1,

                -- Uygun Pozisyonlar (EN ÖNEMLİ)
                suitable_positions TEXT,
                suitable_positions_search TEXT,
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_ci_career ON candidate_intelligence(career_path)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_ci_level ON candidate_intelligence(level)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_ci_company ON candidate_intelligence(company_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_ci_search ON candidate_intelligence(suitable_positions_search)')
                ...
                try:
                    ...
                    if candidate_dict and position_dict:
                        ...
                        if v3_result and v3_result.success:
                            ...
                            # V3 skorlarını al
                            v3_gemini = getattr(v3_result, 'gemini_score', 0) or 0
                            v3_hermes = getattr(v3_result, 'hermes_score', 0) or 0
                            v3_openai = getattr(v3_result, 'openai_score', 0) or 0
                            
                            # candidate_positions'da match_score ve V3 skorları güncelle
                            cursor.execute("""
                                ...
                                WHERE candidate_id = ? AND position_id = ?
                            """, (final_score, v2_score, v3_score, v3_gemini, v3_hermes, v3_openai, candidate_id, position_pool_id))
                            ...
                            if final_score < MINIMUM_MATCH_THRESHOLD:
                                ...
                                # FAZ 12.6: Başka pozisyonu yoksa Genel Havuz'a döndür
                                cursor.execute("SELECT COUNT(*) FROM candidate_positions WHERE candidate_id = ?", (candidate_id,))
                                other_positions = cursor.fetchone()[0]
...
def match_single_candidate_to_positions(candidate_id: int, company_id: int) -> dict:
    """
    Tek adayı firmanın tüm aktif pozisyonlarıyla eşleştirir.
    CV yükleme sonrası çağrılır.
    
    CLAUDE.md Kuralı: Max 1 pozisyon/aday
    
    Args:
        candidate_id: Yeni yüklenen adayın ID'si
        company_id: Firma ID
    ...
    Returns:
        {
            ...
            'matched': int,
            'transferred': int,
            'matched_positions': [{'position_id': int, 'name': str, 'score': int}]
        }
    """
    stats = {
        'candidate_id': candidate_id,
    ...
    if not candidate:
        logger.warning(f"match_single_candidate: Aday {candidate_id} bulunamadı")
        return stats
    
    # Sadece durum='yeni' olan adaylar eşleştirilir
    if candidate.durum != 'yeni':
        logger.info(f"match_single_candidate: Aday {candidate_id} durumu '{candidate.durum}', atlanıyor")
        return stats
    
    # Max 1 pozisyon kontrolü (CLAUDE.md Kural 6)
    current_count = get_candidate_position_count(candidate_id)
    if current_count >= 1:
        logger.info(f"match_single_candidate: Aday {candidate_id} zaten {current_count} pozisyonda")
        return stats
    
    # Candidate dict hazırla (V3 için genişletilmiş - FAZ 10.1.1)
    candidate_dict = {
        'id': candidate_id,
        'ad_soyad': candidate.ad_soyad or '',
        'email': getattr(candidate, 'email', '') or '',
        'telefon': getattr(candidate, 'telefon', '') or '',
        'mevcut_pozisyon': candidate.mevcut_pozisyon or '',
        'mevcut_sirket': getattr(candidate, 'mevcut_sirket', '') or '',
        'teknik_beceriler': candidate.teknik_beceriler or '',
        'toplam_deneyim_yil': candidate.toplam_deneyim_yil or 0,
        'egitim': candidate.egitim or '',
        'universite': getattr(candidate, 'universite', '') or '',
        'bolum': getattr(candidate, 'bolum', '') or '',
        'lokasyon': candidate.lokasyon or '',
        'deneyim_detay': candidate.deneyim_detay or '',
        'deneyim_aciklama': getattr(candidate, 'deneyim_aciklama', '') or '',
        'diller': getattr(candidate, 'diller', '') or '',
        'sertifikalar': getattr(candidate, 'sertifikalar', '') or '',
        'cv_raw_text': getattr(candidate, 'cv_raw_text', '') or ''
    }
    
    # 2. Firmanın aktif pozisyonlarını al
    positions = get_department_pools(company_id, pool_type='position')
    positions = [p for p in positions if p.get('is_active', 1) and p.get('keywords')]
    stats['positions_scanned'] = len(positions)
    
    if not positions:
        return stats
    
    # 3. Her pozisyon için eşleştirme (Max 1 pozisyon - ilk eşleşmede dur)
    from scoring_v2 import get_title_mappings, calculate_match_score_v2
    ...
    for position in positions:
        ...
        # Title match kontrolü
        match_found, title_score = check_title_match(candidate_dict, title_mappings)
        
        if not match_found or title_score == 0:
            continue
        
        # v2 scoring hesapla (V3 için genişletilmiş - FAZ 10.1.1)
        position_dict = {
            'id': position_id,
            'name': position_name,
            'keywords': position.get('keywords', ''),
            'gerekli_deneyim_yil': position.get('gerekli_deneyim_yil', 0),
            'gerekli_egitim': position.get('gerekli_egitim', ''),
            'lokasyon': position.get('lokasyon', ''),
            'aranan_nitelikler': position.get('aranan_nitelikler', ''),
            'is_tanimi': position.get('is_tanimi', ''),
            'gorev_tanimi': position.get('gorev_tanimi', '')
        }
        
        try:
            v2_result = calculate_match_score_v2(candidate_dict, position_dict)
            v2_score = v2_result.get('total', title_score) if v2_result else title_score
        except Exception as e:
            logger.warning(f"v2 scoring hatası: {e}")
            v2_score = title_score
            v2_result = None
        
        # === FAZ 10.1.1: V3 Değerlendirme + Weighted Average ===
        final_score = v2_score  # Varsayılan: sadece V2
        v3_evaluated = False
        
        if v2_result:
            try:
                from core.scoring_v3.evaluate_candidate import evaluate_candidate_sync
                ...
                if v3_result and v3_result.success:
                    v3_score = v3_result.total_score
                    # Weighted Average: (V3 × 0.60) + (V2 × 0.40)
                    final_score = int((v3_score * 0.60) + (v2_score * 0.40))
                    v3_evaluated = True
                    logger.info(f"[match_single] V3: aday={candidate_id}, V2={v2_score}, V3={v3_score}, Final={final_score}")
                    
                    # ai_evaluations tablosuna kaydet
                    ...
                    logger.warning(f"[match_single] V3 başarısız, V2 kullanılıyor: aday={candidate_id}")
            except Exception as v3_err:
                logger.warning(f"[match_single] V3 hatası, V2 kullanılıyor: {v3_err}")
        # === FAZ 10.1.1 SONU ===
        
        stats['matched'] += 1
        
        # V3 sonuçlarını hazırla
        v3_gemini = 0
        v3_hermes = 0
        v3_openai = 0
        v3_total = 0
        ...
        if v3_evaluated and 'v3_result' in dir() and v3_result and v3_result.success:
            v3_total = v3_result.total_score
            v3_gemini = getattr(v3_result, 'gemini_score', 0) or 0
            v3_hermes = getattr(v3_result, 'hermes_score', 0) or 0
            v3_openai = getattr(v3_result, 'openai_score', 0) or 0
            version = 'v3_weighted'
        
        # Pozisyona ekle (final_score ve V3 skorları ile)
        result = add_candidate_to_position(
            ...
            v2_score=v2_score,
            v3_score=v3_total,
            gemini_score=v3_gemini,
            hermes_score=v3_hermes,
            openai_score=v3_openai,
            score_version=version
        )
        
        if result.get('success'):
            stats['transferred'] += 1
            stats['matched_positions'].append({
                'position_id': position_id,
                'name': position_name,
                'score': final_score,
                'v3_evaluated': v3_evaluated
            })
            
            # Max 1 pozisyon - ilk başarılı eşleşmede dur
            logger.info(f"match_single_candidate: Aday {candidate_id} -> Pozisyon {position_id} ({position_name}), final_skor={final_score}")
            break
...
# ═══════════════════════════════════════════════════════════════════════════════
# CANDIDATE INTELLIGENCE FONKSİYONLARI (CV Profil Analizi)
# ═══════════════════════════════════════════════════════════════════════════════

def save_candidate_intelligence(candidate_id: int, company_id: int, intelligence_data: dict) -> bool:
    """
...
def find_candidates_for_position(company_id: int, position_title: str, sectors: list = None, level: str = None, location: str = None, limit: int = 100) -> list:
    ...
    try:
        ...
        query = """
            SELECT ci.*, c.ad_soyad, c.email, c.telefon, c.mevcut_pozisyon
            FROM candidate_intelligence ci
            JOIN candidates c ON ci.candidate_id = c.id
            WHERE ci.company_id = ?
            AND (
                ci.suitable_positions_search LIKE ?
                OR LOWER(ci.career_path) LIKE ?
                OR LOWER(ci.career_path_alternatives) LIKE ?
            )
        """
        params = [company_id, f"%{search_term}%", f"%{search_term}%", f"%{search_term}%"]
        ...
        if location:
            query += " AND (LOWER(ci.current_location) LIKE ? OR LOWER(ci.preferred_locations) LIKE ?)"
            params.extend([f"%{location.lower()}%", f"%{location.lower()}%"])

        if sectors:
            sector_conditions = " OR ".join(["LOWER(ci.sectors) LIKE ?" for _ in sectors])
            query += f" AND ({sector_conditions})"
            params.extend([f"%{s.lower()}%" for s in sectors])
...
def get_intelligence_stats(company_id: int) -> dict:
    ...
    try:
        ...
        return {
            ...
            "pending_analysis": total_candidates - analyzed_candidates,
            "coverage_percent": round((analyzed_candidates / total_candidates * 100) if total_candidates > 0 else 0, 1),
