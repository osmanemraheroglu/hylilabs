...
def check_keyword_match_weighted(keyword, search_text, skills_text, turkish_lower_func, company_id=None):
    ...
    if method == 'exact':
        ...
        # DB'den effective_weight al (match_weight * (1 - ambiguity * 0.3))
        weight = 0.85  # default
        try:
            synonyms = get_synonyms_with_weights(keyword, company_id)
            matched_lower = turkish_lower_func(matched_word) if matched_word else ''
            for s in synonyms:
        ...
        # fuzzy(85%) -> 0.5 + 0.3 * 0.85 = 0.755
        try:
            ratio = int(method.replace('fuzzy(', '').replace('%)', ''))
            weight = 0.5 + 0.3 * (ratio / 100)
        except:
            weight = 0.6
    else:
        weight = 0.5
...
def calculate_match_score_keyword(candidate: dict, position: dict) -> dict:
    ...
    for kw in keywords:
        found, matched_via, match_method, weight = check_keyword_match_weighted(
            kw, search_text, skills_original, turkish_lower, company_id=None
        )
        if found:
